...
public class ProductoDetallesController implements Initializable {
    @FXML
    private Label lblHistorialCambios;
    @FXML
    ...
    @FXML
    private Label txtPrincipioActivo;
    @FXML
    private Label txtConcentracion;
    @FXML
    private Label txtFormaFarmaceutica;
    @FXML
    ...
    @FXML
    private Label txtMargenGanancia;
    @FXML
    private Label txtStockActual;
    @FXML
    private Label txtStockMinimo;
    @FXML
    private Label txtStockMaximo;
    @FXML
    private Label txtUbicacion;
    @FXML
    private Label txtLote;
    @FXML
    private Label txtFechaVencimiento;
    @FXML
    private Label txtLaboratorio;
    @FXML
    private Label txtFechaFabricacion;
    @FXML
    private Label txtProveedor;
    @FXML
    private Label txtRequiereReceta;
    @FXML
    private Label txtEsControlado;
    ...
    private final java.time.format.DateTimeFormatter formatterHistorial = java.time.format.DateTimeFormatter
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (colFecha != null) {
            colFecha.setCellValueFactory(cell -> new javafx.beans.property.SimpleStringProperty(
                    cell.getValue().fecha.toLocalDateTime().format(formatterHistorial)));
        }
        if (colUsuario != null) {
            colUsuario.setCellValueFactory(
                    cell -> new javafx.beans.property.SimpleStringProperty(cell.getValue().usuario));
        }
        if (colCampo != null) {
        ...
        if (colAnterior != null) {
            colAnterior.setCellValueFactory(
                    cell -> new javafx.beans.property.SimpleStringProperty(cell.getValue().anterior));
        }
        if (colNuevo != null) {
            colNuevo.setCellValueFactory(cell -> new javafx.beans.property.SimpleStringProperty(cell.getValue().nuevo));
        }

        // Control de visibilidad por rol: solo Admin ve el historial
        Usuario usuario = ServiceContainer.getInstance().getAuthenticationService().getUsuarioActual();
        boolean esAdmin = usuario != null && usuario.isAdmin();
        if (tableHistorial != null) {
            tableHistorial.setVisible(esAdmin);
            tableHistorial.setManaged(esAdmin);
        }
        if (lblHistorialCambios != null) {
            lblHistorialCambios.setVisible(esAdmin);
            lblHistorialCambios.setManaged(esAdmin);
        }
    }
    ...
    private void cargarHistorial() {
        if (tableHistorial != null && producto != null && producto.getId() != null) {
            java.util.List<ProductoHistorialCambioRepository.HistorialCambioDTO> historial = ProductoHistorialCambioRepository
    ...
    private void cargarDatos() {
        ...
        txtFormaFarmaceutica
                .setText(producto.getFormaFarmaceutica() != null ? producto.getFormaFarmaceutica().toString() : "-");
        ...
        txtFechaVencimiento
                .setText(producto.getFechaVencimiento() != null ? producto.getFechaVencimiento().format(formatoFecha)
                        : "No especificada");
        txtLaboratorio.setText(producto.getLaboratorio() != null && !producto.getLaboratorio().trim().isEmpty()
                ? producto.getLaboratorio()
                : "No especificado");
        txtFechaFabricacion
                .setText(producto.getFechaFabricacion() != null ? producto.getFechaFabricacion().format(formatoFecha)
                        : "No especificada");
