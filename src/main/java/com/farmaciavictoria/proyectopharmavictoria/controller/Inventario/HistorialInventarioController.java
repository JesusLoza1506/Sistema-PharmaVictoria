...
public class HistorialInventarioController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        colFecha.setCellValueFactory(cell -> new javafx.beans.property.SimpleStringProperty(formatter.format(cell.getValue().fecha.toLocalDateTime())));
        ...
        tableHistorial.widthProperty().addListener((obs, oldVal, newVal) -> {
            ...
            if (header != null) {
                header.setStyle("-fx-background-color: linear-gradient(to right, #28a745, #20c997); -fx-text-fill: white; font-weight: bold; -fx-border-color: #20c997; -fx-border-width: 0 0 2px 0;");
            }
        });
    }

    private void cargarHistorial() {
        List<ProductoHistorialCambioRepository.HistorialCambioDTO> historial = ProductoHistorialCambioRepository.obtenerCambiosPorProducto(producto.getId());
