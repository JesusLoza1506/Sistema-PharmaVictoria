...
/**
 * Repository para historial de cambios de usuario
 */
public class UsuarioHistorialCambioRepository {
    ...
    public void save(UsuarioHistorialCambio cambio) {
        ...
        try (Connection conn = databaseConfig.getConnection();
            ...
            stmt.setTimestamp(6, cambio.getFecha() != null ? Timestamp.valueOf(cambio.getFecha()) : new Timestamp(System.currentTimeMillis()));
            stmt.executeUpdate();
            logger.debug("Historial de cambio guardado para usuario_id={}", cambio.getUsuario_id());
        } catch (SQLException e) {
            logger.error("Error al guardar historial de cambio para usuario_id={}: {}", cambio.getUsuario_id(), e.getMessage(), e);
