...
public class ProveedorRepository {
        ...
        private boolean insert(Proveedor proveedor) {
            String sql = """
                INSERT INTO proveedores (razon_social, ruc, contacto, telefono, email, 
                                       direccion, condiciones_pago, observaciones, activo, sucursal_id, tipo_producto)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """;
            try (Connection connection = DatabaseConfig.getInstance().getConnection();
                 PreparedStatement stmt = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                setProveedorParameters(stmt, proveedor);
        ...
        public boolean update(Proveedor proveedor) {
            String sql = """
                UPDATE proveedores SET razon_social=?, ruc=?, contacto=?, telefono=?, 
                                     email=?, direccion=?, condiciones_pago=?, observaciones=?, activo=?, sucursal_id=?, tipo_producto=?
                WHERE id=?
            """;
            try (Connection connection = DatabaseConfig.getInstance().getConnection();
                 PreparedStatement stmt = connection.prepareStatement(sql)) {
     ...
     */
        private Proveedor mapResultSetToProveedor(ResultSet rs) throws SQLException {
            ...
            proveedor.setSucursalId((Integer)rs.getObject("sucursal_id"));
    ...
    /**
     * Establecer parámetros del PreparedStatement para proveedor
     */
        private void setProveedorParameters(PreparedStatement stmt, Proveedor proveedor) throws SQLException {
            ...
            stmt.setObject(10, proveedor.getSucursalId());
            stmt.setString(11, proveedor.getTipoProducto());
        }
    // Métodos para historial de cambios
    public boolean registrarCambioProveedor(int proveedorId, String campo, String valorAnterior, String valorNuevo, String usuario) {
    ...
    public List<HistorialCambioDTO> obtenerHistorialCambios(int proveedorId) {
        ...
        try (Connection connection = DatabaseConfig.getInstance().getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    String nombreCompleto = rs.getString("nombres") != null ? rs.getString("nombres") + " " + rs.getString("apellidos") : rs.getString("usuario");
    ...
    /**
     * Verifica si existe un proveedor con la misma razón social (excluyendo el id si se indica)
     */
    public boolean existePorRazonSocial(String razonSocial, Integer idExcluir) {
        String sql = "SELECT COUNT(*) FROM proveedores WHERE LOWER(razon_social) = LOWER(?)" + (idExcluir != null ? " AND id <> ?" : "");
        try (Connection connection = DatabaseConfig.getInstance().getConnection();
             PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, razonSocial);
            if (idExcluir != null) stmt.setInt(2, idExcluir);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
